...
import com.collager.trillo.util.OpenAIApi;
import com.collager.trillo.util.TrilloFunction;

import java.util.Map;

public class ChatUsingOpenAi implements TrilloFunction {
  ...
  private Result chat(Map<String, Object> params) {
    ...
    // call OpenAiApi to send messages
    LogApi.auditLogInfo("Sending request to chat...");
    res = (Result) OpenAIApi.chat(params);

    // in case of any error log it
    if (res.isFailed()) {
      LogApi.auditLogError("There is an error to get response from OpenAI Model. Error: " + res.getMessage());
      return Result.getFailedResult("There is an error to get response from OpenAI Model");
    }
